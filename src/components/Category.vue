...
<v-flex v-if="filteredMenu.filter(item => item.category_id === category.category_id).length">
      <v-card class="clickable headline grey lighten-2" primary-title>
        <v-card-title primary-title @click="categoryToggled = !categoryToggled" class="display-1" v-if="menuLang === 'english' || menuLang === 'both' "> {{category.eng_category.toUpperCase()}} <v-icon v-if="!categoryToggled" >arrow_drop_down</v-icon> <v-icon v-if="categoryToggled" >arrow_drop_up</v-icon> </v-card-title>
